...
- **0.x.MINOR** — new functionality, *and* any breaking change (removing or
  renaming public API, changed defaults or behavior). Pre-1.0, MINOR carries
  the breaking changes — there is no separate major bump until 1.0.0.
...
Every `ask-*` gem — ask-notion included — and **yamine** is released through
...
gemchain guard ask-notion
gemchain update ask-notion <new-version> --dry-run
gemchain update ask-notion <new-version> --test-only
gemchain update ask-notion <new-version>
```
...
ask-notion depends on `ask-auth`. Releasing ask-auth (or any gem this one
depends on) is never a single-gem event: one
`gemchain update ask-auth <ver>` from the workspace root rewrites this gem's
gemspec constraint, re-runs this suite, bumps ask-notion one patch, commits,
publishes, tags, and pushes — in topological order. No gemspec currently
depends on ask-notion, but `gemchain guard ask-notion` always shows the
current blast radius. Never hand-edit a constraint or release dependents
separately.
...
3. **Build** — `gem build ask-notion.gemspec` succeeds (gemchain does this
...
4. **Changelog** — `## [Unreleased]` entries moved under a new
   ...
   opened above it.
5. **Version** — bumped in `lib/ask/notion/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/notion/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
Starting from `0.1.3`, released via `gemchain update ask-notion <ver>` from
the workspace root:

| Change | Next version | Why |
|---|---|---|
| Fix a page-sync bug | `0.1.4` | pre-1.0 fix ⇒ sequential PATCH |
| Three bug fixes land together | `0.1.5` (one release, not `0.1.6`) | one step, however many fixes |
| Add a new Notion block mapper | `0.2.0` | new feature ⇒ pre-1.0 MINOR |
| Rename a public client method (breaking) | `0.2.0` | breaking ⇒ pre-1.0 MINOR |
| Declare the API stable | `1.0.0` | first stable major |
| `ask-auth` releases | cascade via `gemchain update ask-auth <ver>` | this gem's constraint is rewritten and it is re-tested, bumped one patch, published — never by hand |
